...
- **Domain Expertise**:
  ...
  - **Dynamic Configuration Hot-Reload & IPC**: Expert in SmallRye Config architecture (`SmallRyeConfig`, `ConfigSource`, `ConfigMapping`), Java 17+ UNIX Domain Socket channels (`UnixDomainSocketAddress`), Gizmo AOT bytecode proxy generation, atomic snapshot swapping, and non-disruptive configuration reload mechanisms.
- **Core Architectural Principles**:
  ...
  - **Zero-Overhead & Invariant-Preserving Hot Reload**: Hot configuration reloads dispatch via AOT-compiled bytecode proxies with atomic snapshot swapping (`AtomicReference`) rather than reflective lookup or heavy dynamic interception, and must validate the full snapshot before applying changes, ensuring failure does not leave state corrupted.
...
- **Modular Architecture (Deployment vs. Runtime)**:
  ...
  - `runtime`: Contains runtime configuration (`@ConfigMapping`), bytecode recorders (`@Recorder`), the control socket server (`ControlSocketServer`), config reload services (`ConfigReloadService`), and reloadable config registry (`ReloadableConfigRegistry`). Dependencies MUST remain minimal (only `quarkus-core`).
