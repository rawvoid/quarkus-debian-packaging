...
### Hot-Path Execution & Atomic Safety Invariants

Configuration reloading is built with **AOT Gizmo-generated bytecode proxies with direct `AtomicReference` holder binding**:

- **AOT Proxy Generation**: Config proxies (`<Interface>$$ReloadProxy`) are compiled during Quarkus augmentation; calls dispatch via standard monomorphic interface invocations.
- **Direct Holder Binding**: Each proxy holds a `private final AtomicReference<Object> holder` bound at instantiation time; property reads execute as a volatile reference read (`holder.get()`).
- **Strict Atomic Pointer Swapping**: Updates replace the configuration snapshot atomically (`AtomicReference.set()`), ensuring readers observe complete and valid state snapshots.
- **Transparent CDI Integration**: Proxies are registered as Arc `@Alternative` synthetic beans with `@Priority(1000)`, automatically satisfying `@Inject` injection points.
