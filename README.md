...
A Quarkus extension that builds production-ready, Debian binary packages (`.deb`) and provides zero-downtime configuration hot reload via UNIX domain sockets and atomic snapshot swapping.
...
- **Linux FHS & Systemd Integration**: Conforms to the Linux Filesystem Hierarchy Standard (`/usr/share/<app>`, `/etc/<app>`, `/var/log/<app>`, `/var/lib/<app>`, `/run/<app>`), installs systemd unit files (`/usr/lib/systemd/system/<app>.service`), sets up dedicated non-root service accounts, and configures ambient network capabilities (`AmbientCapabilities=CAP_NET_BIND_SERVICE`, `CapabilityBoundingSet=CAP_NET_BIND_SERVICE`) to allow unprivileged binding to privileged ports (< 1024).
- **Zero-Downtime Configuration Hot Reload**:
  ...
  - **Zero Hot-Path Overhead**: Direct method delegation via AOT Gizmo-generated bytecode proxies backed by `AtomicReference` snapshots. Property reads execute via volatile reference reads without synchronization locks.
...
| `/etc/<app>/application.properties` | `root:<app> 0640` | External base configuration file (SmallRye ordinal 275, hot-reloadable) |
...
| `quarkus.debian.architecture` | `all` (JVM) / Linux ELF header (native) | Target CPU architecture (`all`, `amd64`, `arm64`, `i386`, `ppc64el`, `ppc64`, `s390x`, `riscv64`). For native packages, the architecture is determined directly from the Linux ELF binary header, failing fast if the binary is missing, non-ELF, or unsupported. |
| `quarkus.debian.depends` | (Computed dynamically) | Additional package dependencies to include in the Debian control Depends field. Base dependencies (`systemd`, JRE for JVM, `python3` when reload is enabled) are computed automatically. |
...
- **Symmetric Proxy Equality**: Proxy `equals(Object)` strictly requires `instanceof <ProxyClassName>` and delegates to `Objects.equals()`, guaranteeing mathematical symmetry and preventing equality collisions with raw configuration mapping classes.

### External Configuration Hierarchy & Activation Latch

- **Ordinal Hierarchy**:
  - Bundled packaged properties: `250`
  - External base configuration (`/etc/<app>/application.properties`): `275`
  - External profile configuration (`/etc/<app>/application-<profile>.properties`): `275 + profiles.size() - i < 300`
  - System environment variables: `300`
  - JVM system properties (`-Dkey=value`): `400`
- **Activation Latch (`QUARKUS_DEBIAN_EXTERNAL_CONFIG`)**:
  External configuration files are only loaded when `QUARKUS_DEBIAN_EXTERNAL_CONFIG=true` is exported. The production launcher (`/usr/bin/<app>`) exports this variable after sourcing `/etc/default/<app>`, ensuring build-time packaging (`mvn package`), development mode (`quarkus:dev`), and test suites (`@QuarkusTest`) never accidentally read host `/etc` files.
- **Reload Trigger (`ExecReload`)**:
  `systemctl reload <app>` executes `ExecReload=${executableFile} --reload`, delegating to `<installDir>/reload` (an embedded pure Python 3 script `reload.py`). The script communicates over the UNIX domain socket `/run/<app>/control.sock`. When hot reload is enabled, `python3` is automatically included as a Debian package dependency; when disabled, the packaging has zero Python dependencies.
