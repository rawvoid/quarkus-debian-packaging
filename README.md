...
+---------------------------------------+
   ...
   [Valid]                    [Invalid]
      ...
      v                           v
+------------------------+  +---------------------------+
| Atomic Snapshot Swap   |  | Reject & log errors       |
| ReloadableConfig-      |  | Live state UNTOUCHED      |
| Registry.swap(...)     |  +---------------------------+
+------------------------+
      |
      v
...
### Hot-Path Zero-Overhead & Atomic Safety Guarantee

Unlike reflection-based field patching or runtime dynamic invocation handlers, `quarkus-debian-packaging` uses **AOT Gizmo-generated bytecode proxies with direct `AtomicReference` holder binding**:

- **Zero Runtime Reflection**: Config proxies (`<Interface>$$ReloadProxy`) are generated during Quarkus augmentation; hot paths execute direct interface calls with zero reflection.
- **Zero Heap Allocations & Zero Map Lookups**: Each proxy directly holds a `private final AtomicReference<Object> holder` bound at instantiation time. Reading properties executes a volatile read (`holder.get()`) without map hashing or heap allocations.
- **Strict Atomic Pointer Swapping**: Updates swap the entire configuration snapshot atomically (`AtomicReference.set()`). Readers never observe partially updated intermediate state.
- **Transparent CDI Integration**: Proxies are registered as Arc `@Alternative` synthetic beans with `@Priority(1000)`. Application beans (`@Inject MyConfig`) receive the reloadable proxy without changing business code.
